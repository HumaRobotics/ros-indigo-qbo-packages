...
CInfraRedsController::CInfraRedsController(std::string name, CQboduinoDriver *device_p, ros::NodeHandle& nh) : CController(name,device_p,nh)
{
    ...
    irs_pub_ = nh.advertise<std_msgs::UInt8MultiArray>("irs_state", 1);
...
//ROS_WARN_STREAM("IR timer callback");
	...
	msg.data.push_back(ir0_);
	msg.data.push_back(ir1_);
	msg.data.push_back(ir2_);
